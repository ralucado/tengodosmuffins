...
void Player::kill() {
    if(dead || revivingTimer != 0.0f) return;
    dead = true;
    deadTimer = 2.0f;
}
...
void Player::update(float deltaTime) {
    if(!dead) {
        ...
        revivingTimer = std::max(0.0f, revivingTimer-deltaTime);
        ...
        deadTimer = std::max(deadTimer-deltaTime, 0.0f);
        dead = (deadTimer != 0.0f);
        if(!dead) {
            revivingTimer = 3.0f;
        }
...
void Player::draw(sf::RenderWindow* window) {
    if(revivingTimer > 0.0f && std::fmod(revivingTimer, .4f) > 0.2f)
        return;
    window->draw(backTex);
    window->draw(*this);
}
